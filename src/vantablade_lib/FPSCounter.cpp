...
void FPSCounter::frameInTitle(const bool vsync, const bool showMax) {
    ...
    if (showMax) {
        titleBuffer = FORMAT("{} - {:.3LF} fps/{} - Max: {:.3LF} - VSync: {}",
                               m_title, fps, ms_per_frameComposition, max_fps, vsyncText);
    } else {
        titleBuffer = FORMAT("{} - {:.3LF} fps/{} - VSync: {}",
                               m_title, fps, ms_per_frameComposition, vsyncText);
    }
...
void FPSCounter::updateFPS() noexcept {
    ...
    ++frames;
    const auto time_step = vnd::TimeValues(
        ch::duration_cast<vnd::nanolld>(current_time - last_time).count());
...
void FPSCounter::ensureMsPerFrameStringUpToDate() {
    if (ms_per_frame == ms_per_frame_last_formatted) {
        return;
    }
...
void FPSCounter::formatTimeInto(std::string& out, const long double inputTimeMilli) {
    ...
    out = FORMAT("{}ms,{}us,{}ns",
                 C_LD(durationMs.count()),
                 C_LD(durationUs.count()),
                 C_LD(durationNs.count()));
}
DISABLE_WARNINGS_POP()

long double FPSCounter::getFPS() const noexcept {
    return fps;
}

long double FPSCounter::getMsPerFrame() const noexcept {
    return ms_per_frame;
}
