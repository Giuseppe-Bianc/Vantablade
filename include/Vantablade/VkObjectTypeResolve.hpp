...
// ── 5.5  EXT / vendor extension handles ──────────────────────────────────────
    ...
    // setVulkanDebugName<T> assigns a human-readable label to any supported
    // Vulkan handle using VK_EXT_debug_utils. The object type is resolved purely
    // at compile time; there is no branching or lookup in generated machine code.
    //
    // Prerequisites:
    //   • VK_EXT_debug_utils enabled at VkInstance creation.
    //   • pfnSetDebugUtilsObjectName loaded via vkGetDeviceProcAddr.
    //
    // Example:
    //   vkutil::setVulkanDebugName(device, myBuffer,
    //                              "SceneVertexBuffer",
    //                              pfnSetDebugUtilsObjectName);

    template <typename T>
    inline VkResult setVulkanDebugName(VkDevice device, T handle, const char *name,
                                       PFN_vkSetDebugUtilsObjectNameEXT pfnSetDebugUtilsObjectName) noexcept {
        // Compile-time constant — folded directly into the struct literal.
        constexpr VkObjectType objectType = vulkanObjectType<T>();

        const VkDebugUtilsObjectNameInfoEXT nameInfo{
            .sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_NAME_INFO_EXT,
            .pNext = nullptr,
            .objectType = objectType,
            // reinterpret_cast to uint64_t is required by the Vulkan spec
            // for passing an opaque handle into the debug-utils API (§3.3).
            .objectHandle = reinterpret_cast<std::uint64_t>(handle),
            .pObjectName = name,
        };

        return pfnSetDebugUtilsObjectName(device, &nameInfo);
    }

    // ─── Section 7: Extension Guide ──────────────────────────────────────────────
    // To add support for a new Vulkan handle type, e.g. a future
    // VkSomeNewHandleXXX introduced by VK_XXX_some_extension:
    //
    //  1. Wrap the specialization in the appropriate #if defined(VK_XXX_...) guard
    //     so the file compiles cleanly against older SDK versions that do not
    //     expose the handle:
    //
    //       #if defined(VK_XXX_some_extension)
    //       template <> constexpr VkObjectType
    //       vulkanObjectType<VkSomeNewHandleXXX>() noexcept
    //           { return VK_OBJECT_TYPE_SOME_NEW_HANDLE_XXX; }
    //       #endif
    //
    //  2. Add the matching guarded static_assert in Section 5:
    //
    //       #if defined(VK_XXX_some_extension)
    //       static_assert(vulkanObjectType<VkSomeNewHandleXXX>()
    //                     == VK_OBJECT_TYPE_SOME_NEW_HANDLE_XXX);
    //       #endif
    //
    //  3. Update the coverage list in the Section 1 comment block.
    //
    //  No other changes are required; setVulkanDebugName gains support for the
    //  new type automatically because it is templated on T.
